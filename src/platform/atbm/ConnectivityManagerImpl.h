...
private:
    ...
    void OnStationIPv4AddressAvailable(void);//(const ip_event_got_ip_t & got_ip);
    void OnStationIPv4AddressLost(void);
    void OnIPv6AddressAvailable(void);//(const ip_event_got_ip6_t & got_ip);
 ...
 * that are specific to the ATBM platform.
 */
