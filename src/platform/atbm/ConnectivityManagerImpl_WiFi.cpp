...
void ConnectivityManagerImpl::ChangeWiFiStationState(WiFiStationState newState)
{
    if (mWiFiStationState != newState)
    {
        ...
        SystemLayer().ScheduleLambda([]() { NetworkCommissioning::ATBMWiFiDriver::GetInstance().OnNetworkStatusChange(); });
    }
}
...
void ConnectivityManagerImpl::OnIPv6AddressAvailable(void)//(const ip_event_got_ip6_t & got_ip)
{
#if 0//CHIP_PROGRESS_LOGGING
    {
        ChipLogProgress(DeviceLayer, "IPv6 addr available. Ready on %s interface: " IPV6STR, IPV62STR(got_ip.ip6_info.ip));
    }
#endif // CHIP_PROGRESS_LOGGING
