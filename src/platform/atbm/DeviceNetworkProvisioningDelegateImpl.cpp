...
CHIP_ERROR DeviceNetworkProvisioningDelegateImpl::_ProvisionWiFiNetwork(const char * ssid, const char * key)
{
    ...
    ChipLogProgress(NetworkProvisioning, "ATBMNetworkProvisioningDelegate: SSID: %s", ssid);
    err = NetworkCommissioning::ATBMWiFiDriver::GetInstance().ConnectWiFiNetwork(ssid, strlen(ssid), key, strlen(key));
